...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) throw Exception(fmt::format("statement errored with func call {}", func_name));
  if (args[0]->GetReturnType().GetType() != TypeId::VARCHAR)
    throw Exception(fmt::format("statement errored with func call {}", func_name));
